...
// อัปเดตแถว (ใช้คีย์ลับ) คืนแถวที่ถูกอัปเดต
export async function sbPatch(path, body) {
  return (await sbFetch(path, { secret: true, method: 'PATCH', body, prefer: 'return=representation' })) || [];
}
...
export function sessionToOrder(s) {
  ...
  return {
    ...
    product_name: m.productName ? (m.bumpProductName ? `${m.productName} + ${m.bumpProductName}` : m.productName) : null,
    slug: m.slug || null,
